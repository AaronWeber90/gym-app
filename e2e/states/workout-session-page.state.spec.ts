...
async function seedWorkoutSessionData(
	page: Page,
	payload: { id: string; sessionId: string; date: string },
) {
	await page.goto("/#/workouts");
	await page.evaluate(async ({ id, sessionId, date }) => {
...
async function assertSessionPageState(
	page: Page,
	workoutId: string,
	sessionId: string,
) {
...
test("workout session detail page state exposes key accessible elements", async ({
	...
	await seedWorkoutSessionData(page, {
		id: workoutId,
		sessionId,
		date: sessionDate,
	});
	await assertSessionPageState(page, workoutId, sessionId);
});
